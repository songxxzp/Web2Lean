        ...
        try:
            ...
            for row in main_cursor.fetchall():
                hist_cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'historical', ?)
                ''', (
                    row['question_id'], row['title'], row['body'], row['tags'],
                    row['score'], row['view_count'], row['answer_count'],
                    row['creation_date'], row['last_activity_date'], row['owner'],
                    row['link'], row['is_answered'], row['accepted_answer_id'],
                    row['creation_date'], row['last_activity_date'], update_id
                ))
            ...
            for row in main_cursor.fetchall():
                hist_cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'historical', ?)
                ''', (
                    row['answer_id'], row['question_id'], row['body'],
                    row['score'], row['creation_date'], row['last_activity_date'],
                    row['owner'], row['is_accepted'], row['creation_date'],
                    row['last_activity_date'], update_id
                ))
...
class DataUpdater:
    ...
    def check_question_updates(self, question_id: int) -> Tuple[bool, Question, List[Answer]]:
        """检查问题是否有更新"""
        # 获取最新的问题数据
        latest_question = self.crawler.db_manager.get_question_by_id(question_id)
